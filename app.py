...
if check_password():
    ...
    for i, (name, symbol) in enumerate(m_tickers.items()):
        ...
        try:
            ...
            cols[col_idx].metric(name, f"{val:,.2f} $")
        except:
            cols[col_idx].metric(name, "N/A")

    st.divider()

    # WICHTIG: Daten laden, bevor sie benutzt werden
    df_base = load_data()
    ...
    if not df_base.empty:
        ...
        m1.metric("Gesamtwert Portfolio", f"{df['Wert'].sum():,.2f} $")
        m2.metric("Gesamt Investiert", f"{df['Invest'].sum():,.2f} $")
        ges_profit = df['Profit'].sum()
        ges_perf = (ges_profit / df['Invest'].sum() * 100) if df['Invest'].sum() > 0 else 0
        m3.metric("Gesamt Profit", f"{ges_profit:,.2f} $", f"{ges_perf:+.2f}%")
        ...
        with t1:
            ...
            c1, c2 = st.columns(2)
            c1.plotly_chart(px.pie(df, values='Wert', names='typ', title="Klassen"), use_container_width=True)
            c2.plotly_chart(px.pie(df, values='Wert', names='ticker', title="Assets"), use_container_width=True)
        ...
        with t5:
            ...
            if not h_df.empty:
                ...
                st.metric("Realisierter Gewinn Gesamt", f"{h_df['gewinn'].sum():,.2f} $")
